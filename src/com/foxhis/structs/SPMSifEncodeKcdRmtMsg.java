...
/**
*/
public class SPMSifEncodeKcdRmtMsg  {
	public SPMSifEncodeKcdRmtMsg() {
	}
	
	public SPMSifEncodeKcdRmtMsg(SPMSifEncodeKcdRmtMsg source) {
		if(source == null) {
	...
	public SPMSifEncodeKcdRmtMsg clone() {
		return new SPMSifEncodeKcdRmtMsg(this);
	}
	
	public static SPMSifEncodeKcdRmtMsg read(java.nio.ByteBuffer buf) throws java.io.IOException {
		return read(buf, false);
	}
	
	public static SPMSifEncodeKcdRmtMsg read(java.nio.ByteBuffer buf, boolean partialRead) throws java.io.IOException {
		if(buf.remaining() == 0) {
			// avoid empty object construction for partial reads
			throw new java.nio.BufferUnderflowException();
		}
		
		SPMSifEncodeKcdRmtMsg obj = new SPMSifEncodeKcdRmtMsg();
		...
		obj.setDd(readDd(buf, partialRead));
		
		obj.setSs(readSs(buf, partialRead));
	...
	*/
	...
	public String toString() {
		StringBuilder buf = new StringBuilder("com.foxhis.structs.SPMSifEncodeKcdRmtMsg[");
		buf.append("hdr1=" + getHdr1());buf.append(", ");
		buf.append("ff=" + getFf());buf.append(", ");
		buf.append("Dta=" + getDta());buf.append(", ");
		buf.append("dd=" + getDd());buf.append(", ");
		buf.append("ss=" + getSs());buf.append(", ");
	...
	public int hashCode() {
		...
		result = prime * result + ((this.ss == null) ? 0 : this.ss.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SPMSifEncodeKcdRmtMsg other = (SPMSifEncodeKcdRmtMsg) obj;
	...
	private static String readDd(java.nio.ByteBuffer buf, boolean partialRead) throws java.io.IOException {
		try {
		byte[] tmp = new byte[3];
		buf.get(tmp);
		int terminatingZeros = "\0".getBytes("UTF-8").length;
	...
	private static String readSs(java.nio.ByteBuffer buf, boolean partialRead) throws java.io.IOException {
		try {
		byte[] tmp = new byte[3];
		buf.get(tmp);
		int terminatingZeros = "\0".getBytes("UTF-8").length;
	...
	private void writeDd(java.nio.ByteBuffer buf) throws java.io.IOException {
		try {
			byte[] encoded = getDd().getBytes("UTF-8");
			int len = Math.min(encoded.length, 3);
			int pad = 3 - len;
			buf.put(encoded, 0, len);
			if(pad > 0) {
				for(int i = 0; i < pad; ++i) {
	...
	private void writeSs(java.nio.ByteBuffer buf) throws java.io.IOException {
		try {
			byte[] encoded = getSs().getBytes("UTF-8");
			int len = Math.min(encoded.length, 3);
			int pad = 3 - len;
			buf.put(encoded, 0, len);
			if(pad > 0) {
